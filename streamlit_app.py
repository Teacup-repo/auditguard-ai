...
with tab_export:
    if findings.empty:
        ...
        # Guard: if data isn't loaded yet, avoid KeyError on 'username'
...
"""
            ...
            st.download_button("⬇️ Download Executive Summary (.md)", summary.encode("utf-8"),
                               file_name="AuditGuard_Executive_Summary.md", mime="text/markdown")

            # Findings CSV export
            csv = findings.to_csv(index=False).encode("utf-8")
            st.download_button("⬇️ Export Findings CSV", csv,
                               file_name="auditguard_findings.csv", mime="text/csv")
            ...
            st.download_button("⬇️ Download Remediation Plan (.md)",
                               fixplan_md.encode("utf-8"),
                               file_name="AuditGuard_Remediation_Plan.md",
                               mime="text/markdown")

            # (Your Framework Legend block can remain below this)
